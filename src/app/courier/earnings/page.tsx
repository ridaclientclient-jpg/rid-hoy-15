...
export default function CourierEarnings() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Get real weekly data from deliveries
      ...
      // Fetch real transactions from courier_transactions or fallback to deliveries
      if (courier?.id) {
        ...
        if (txData && txData.length > 0) {
          const mapped: TxDisplay[] = txData.map((tx: any) => ({
            id: tx.id,
            desc: tx.description || (tx.type === 'withdrawal' ? 'Retiro a banco' : `Entrega`),
            amount: (tx.type === 'withdrawal' || tx.type === 'adjustment') ? -Math.abs(tx.amount) : tx.amount,
            time: formatRelativeTime(tx.created_at),
            type: tx.type === 'withdrawal' ? 'withdraw' : 'delivery',
          }));
          setTransactions(mapped);
        } else {
          // Fallback: build transactions from recent deliveries
          const { data: deliveryTx } = await supabase
            .from('deliveries')
            .select('id, delivery_fee, total, delivery_address, created_at, status')
            .eq('courier_id', courier.id)
            .in('status', ['delivered', 'cancelled'])
            .order('created_at', { ascending: false })
            .limit(15);

          if (deliveryTx && deliveryTx.length > 0) {
            const mapped: TxDisplay[] = deliveryTx.map((d: any) => ({
              id: d.id,
              desc: `Entrega - ${d.delivery_address?.slice(0, 30) || 'Direccion'}`,
              amount: d.status === 'cancelled' ? 0 : (d.delivery_fee || d.total * 0.2),
    ...
    <div className="p-4 space-y-4">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-gray-400">
            <span className="text-xs">{todayDeliveries} entregas</span>
          </div>
        </div>
        {/* Daily Goal */}
      ...
      >
        ...
        <div className="glass rounded-xl p-4">
          ...
          <button
            onClick={() => toast.info('Para retirar fondos, contacta soporte')}
        ...
        <div className="glass rounded-xl p-4">
          ...
          <div className="flex items-center gap-1 mt-2 text-emerald-400">
            ...
            <span className="text-[10px] font-medium">{totalDeliveries} entregas</span>
          </div>
        </div>
      </motion.div>
          ...
          <div className="flex items-end gap-2 h-28">
            {weeklyData.map((data, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: data.amount > 0 ? `${(data.amount / maxAmount) * 100}%` : '4px' }}
                  transition={{ duration: 0.6, delay: i * 0.08 }}
                  className={`w-full rounded-t-lg ${data.amount > 0 ? `bg-gradient-to-t ${data.color} opacity-80 hover:opacity-100` : 'bg-white/10'} transition-opacity cursor-pointer`}
                />
      ...
      >
        ...
        {transactions.length > 0 ? (
          ...
          <div className="glass rounded-2xl p-8 text-center">
            ...
            <p className="text-sm text-gray-500">Sin transacciones aun</p>
            <p className="text-xs text-gray-600 mt-1">Tus ganancias por entregas apareceran aqui</p>
          </div>
        )}
      </motion.div>
    </div>
