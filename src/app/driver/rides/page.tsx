...
// ─── Incoming Ride Sound System ─────────────────────────
let _incomingSoundInterval: ReturnType<typeof setInterval> | null = null;

function playIncomingSoundLoop() {
  try {
    ...
    function playCycle() {
      try {
        ...
        // Urgent 3-tone ascending pattern — louder and longer
        const pattern = [
    ...
    playCycle();
    _incomingSoundInterval = setInterval(playCycle, 3000);
  } catch {}
}

function stopIncomingSoundLoop() {
  if (_incomingSoundInterval) {
    clearInterval(_incomingSoundInterval);
    _incomingSoundInterval = null;
  }
}
...
export default function DriverRides() {
  ...
  const [riderProfile, setRiderProfile] = useState<{ name: string; phone: string; avatar_url?: string; rating: number } | null>(null);
  ...
  // Keep a ref for coords so Realtime subscription can access latest coords
  // without re-subscribing on every GPS update
  ...
  // Initialize from store so navigating from home page keeps connected state
  useEffect(() => {
  ...
  // Realtime subscription to driver status changes (from home page or other sessions)
  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const online = updated.status === 'online' || updated.status === 'busy';
        setIsOnline(online);
      })
      .subscribe();
  ...
  // Toggle online/offline from rides page (same as home page)
  ...
  const fetchRiderProfile = useCallback(async (riderId: string) => {
    try {
      const { data } = await supabase.from('profiles').select('name, phone, avatar_url').eq('id', riderId).single();
      if (data) {
        ...
        setRiderProfile({ name: data.name, phone: data.phone || '', avatar_url: data.avatar_url, rating: Math.round(avg * 10) / 10 });
      }
    } catch {}
  }, []);
  ...
  // Use stable channel name (NOT Date.now()) so channel survives re-renders
  useEffect(() => {
    if (!isOnline || activeRide || incomingRide) return;

    const channel = supabase
      .channel(`driver-searching-rides-${driver?.id || 'unknown'}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'rides', filter: `status=eq.searching` },
        async (payload: any) => {
          const ride = payload.new;
          if (!ride) return;
          // Use ref for coords so we don't need userCoords in dependency array
          const coords = userCoordsRef.current;
          if (!coords) return;

          // Check distance to ride origin
          const distKm = ride.origin_lat && ride.origin_lng
            ? haversineKm(coords.lat, coords.lng, ride.origin_lat, ride.origin_lng)
            : 999;
          ...
          // Fetch rider name, phone, and avatar
          let riderName = 'Pasajero';
          let riderPhone = '';
          let riderAvatar = '';
          try {
            const { data: profile } = await supabase.from('profiles').select('name, phone, avatar_url').eq('id', ride.rider_id).single();
            if (profile) {
              riderName = profile.name;
              riderPhone = profile.phone || '';
              riderAvatar = profile.avatar_url || '';
            }
          } catch {}

          setIncomingRide({
            id: ride.id, rider_id: ride.rider_id, rider_name: riderName, rider_phone: riderPhone, rider_avatar: riderAvatar,
          ...
          if (soundEnabled) playIncomingSoundLoop();
  ...
  // Note: userCoords NOT in deps — we read it via ref to avoid destroying the channel on every GPS tick
  }, [isOnline, activeRide, incomingRide, soundEnabled, driver?.id]);
  ...
  const openGoogleMaps = useCallback(() => {
    ...
    const address = activeRide.status === 'started' ? activeRide.destination : activeRide.origin;
    if (lat && lng) window.open(`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`, '_blank');
    else window.open(`https://www.google.com/maps/search/${encodeURIComponent(address + ', Costa Rica')}`, '_blank');
  }, [activeRide]);
  ...
  const openWaze = useCallback(() => {
    ...
    else window.open('https://waze.com', '_blank');
  }, [activeRide]);
  ...
  // Stop sound loop on unmount
  useEffect(() => {
    return () => { stopIncomingSoundLoop(); };
  }, []);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <button
            ...
            className={`w-full py-3 rounded-full text-[13px] font-medium flex items-center justify-center gap-2 transition-all ${
              isOnline
          ...
          >
            {isToggling ? (
              <>
                ...
                {isOnline ? 'Desconectando...' : 'Conectando...'}
              </>
            ) : isOnline ? (
              <>
                <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                Conectado — recibiendo solicitudes
              </>
            ) : (
              <>
                <div className="w-2 h-2 rounded-full bg-cyan-400" />
                {activeRide ? 'Viaje en curso' : 'Toca para conectarte'}
              </>
            )}
          </button>
        </motion.div>
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  {incomingRide.rider_avatar ? (
                    <img src={incomingRide.rider_avatar} alt={incomingRide.rider_name} className="w-10 h-10 rounded-full object-cover border-2 border-white/10" />
                  ) : (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="glass-strong rounded-2xl p-5 space-y-4 border border-cyan-500/30 glow-cyan">
              ...
              <div className="flex items-center gap-3">
                {riderProfile?.avatar_url ? (
                  <img src={riderProfile.avatar_url} alt={riderProfile.name} className="w-10 h-10 rounded-full object-cover border-2 border-white/10" />
                ) : (
      ...
      {/* Map Choice Modal (Google Maps / Waze) */}
      <AnimatePresence>
        {showMapChoice && (
          <>
            <motion.div className="fixed inset-0 bg-black/60 z-[60]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowMapChoice(false)} />
            <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold text-white">Elegir aplicacion de navegacion</h3>
                <button onClick={() => setShowMapChoice(false)} className="p-1 rounded-lg hover:bg-white/10 transition-colors"><XIcon className="w-5 h-5 text-gray-400" /></button>
              </div>
              <div className="space-y-3">
                <button
                  onClick={openGoogleMaps}
                  className="w-full flex items-center gap-4 p-4 rounded-2xl bg-white/[0.05] border border-white/[0.06] hover:bg-white/[0.08] active:scale-[0.98] transition-all"
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-500">Navegacion con trafico en tiempo real</p>
                  </div>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-500">Rutas optimizadas para conductores</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-500 ml-auto" />
                </button>
              </div>
